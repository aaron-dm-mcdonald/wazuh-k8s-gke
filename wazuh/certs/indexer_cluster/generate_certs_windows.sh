...
openssl x509 -days 3650  -in admin.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out admin.pem
...
openssl x509 -days 3650 -new -key node-key.pem -out node.csr -subj "//C=US\L=California\O=Company\CN=indexer"

echo "create: node.pem"

openssl x509 -days 3650 -in node.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out node.pem
...
openssl x509 -days 3650 -in dashboard.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out dashboard.pem
...
openssl req -x509 -days 3650 -new -key filebeat-key.pem -out filebeat.csr -subj "//C=US\L=California\O=Company\CN=filebeat"

echo "create: filebeat.pem"

openssl x509 -days 3650 -in filebeat.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out filebeat.pem